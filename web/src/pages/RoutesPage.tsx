        ...
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-base whitespace-nowrap" style={{ color: t.text }}>Route Network</span>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value as any)}
              className="px-3 py-2 rounded-xl text-sm outline-none transition-all duration-200"
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          {[
            ...
            <div key={s.label}
              className="p-4 rounded-2xl text-center transition-all duration-200 hover:scale-[1.02]"
              style={{ background: t.card, border: `1px solid ${t.border}` }}>
              <div className="text-2xl font-bold mb-1 truncate" style={{ color: s.color }}>{s.value}</div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="rounded-2xl overflow-hidden"
            ...
            <div className="px-4 sm:px-5 py-3.5 flex items-center justify-between"
              ...
              <Link to="/flights"
                className="text-xs font-semibold px-3 py-1.5 rounded-xl text-white transition-all duration-200"
            ...
            <div className="divide-y" style={{ borderColor: t.border }}>
              {filtered.map((route, i) => (
                <div key={route.id}
                  className="px-4 sm:px-5 py-4 flex items-center gap-3 sm:gap-4 transition-all duration-200"
                  ...
                  <div className="flex-1 flex items-center gap-2 sm:gap-3 min-w-0">
                    <div className="text-center flex-shrink-0">
                      <div className="text-base sm:text-lg font-bold" style={{ color: t.text }}>{route.depIcao}</div>
                      <div className="text-xs truncate max-w-16 sm:max-w-20" style={{ color: t.textMuted }}>{route.depName}</div>
                    </div>
                    <div className="flex-1 flex flex-col items-center gap-1 min-w-0">
                      ...
                      <span className="text-xs whitespace-nowrap" style={{ color: t.textMuted }}>{route.distance} nm</span>
                    </div>
                    <div className="text-center flex-shrink-0">
                      <div className="text-base sm:text-lg font-bold" style={{ color: t.text }}>{route.arrIcao}</div>
                      <div className="text-xs truncate max-w-16 sm:max-w-20" style={{ color: t.textMuted }}>{route.arrName}</div>
                    </div>
                  </div>
